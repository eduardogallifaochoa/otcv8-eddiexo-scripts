...
### Recommended flow (single file only)
1. Download `druid_toolkit_single.lua` (artifact or repo).
2. Copy it to target profile folder.
3. If that profile already autoloads it, just start OTCv8.
4. If not autoloaded, run once in Ingame Script Editor:
```lua
dofile('druid_toolkit_single.lua')
```
