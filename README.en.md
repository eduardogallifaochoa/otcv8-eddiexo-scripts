...
### Option A (recommended autoload)
1. Copy `druid_toolkit_single.lua` into that profile folder (same level as `_Loader.lua`).
2. Make sure `_Loader.lua` has this line (already included in this repo):
```lua
pcall(dofile, 'druid_toolkit_single.lua')
```
3. Start OTCv8 with that profile. Toolkit loads automatically.

### Option B (manual from Ingame Script Editor)
```lua
dofile('druid_toolkit_single.lua')
```
